...
export default function AIModelPanel() {
  ...
  return (
    <div className="flex flex-col gap-4 text-[#431407]">
      {/* Navigation Sub-Tabs */}
      <div className="flex items-center gap-1.5 p-1 rounded-xl bg-[#fff7ed] border border-[#fed7aa]">
        {[
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
        <div className="p-2.5 rounded-xl border bg-white border-[#fed7aa] flex flex-col justify-between">
          <span className="text-[10px] font-bold uppercase tracking-wider text-[#7c2d12]">Production Model</span>
          <span className="text-[13px] font-black text-[#c2410c] truncate">
            {metrics?.version || 'FLAREX CatBoost v2.4'}
          </span>
          <span className="text-[9.5px] font-medium text-emerald-700 flex items-center gap-1 mt-1">
            <CheckCircle2 size={11} /> Verified Holdout
          </span>
        </div>

        <div className="p-2.5 rounded-xl border bg-white border-[#fed7aa] flex flex-col justify-between">
          <span className="text-[10px] font-bold uppercase tracking-wider text-[#7c2d12]">Industrial Fire Recall</span>
          <span className="text-[16px] font-black text-red-600 font-mono">
            {metrics?.industrialFireMetrics ? `${metrics.industrialFireMetrics.recall}%` : '100%'}
          </span>
          <span className="text-[9.5px] text-[#7c2d12]">Zero Missed Crises</span>
        </div>

        <div className="p-2.5 rounded-xl border bg-white border-[#fed7aa] flex flex-col justify-between">
          <span className="text-[10px] font-bold uppercase tracking-wider text-[#7c2d12]">Macro F1 Score</span>
          <span className="text-[16px] font-black text-[#ea580c] font-mono">
            {metrics?.overallMetrics ? `${metrics.overallMetrics.macroF1}%` : '100%'}
          </span>
          <span className="text-[9.5px] text-[#7c2d12]">5-Class Multi-Taxonomy</span>
        </div>

        <div className="p-2.5 rounded-xl border bg-white border-[#fed7aa] flex flex-col justify-between">
          <span className="text-[10px] font-bold uppercase tracking-wider text-[#7c2d12]">Training Benchmark</span>
          <span className="text-[16px] font-black text-[#431407] font-mono">
            {metrics?.dataset ? `${metrics.dataset.totalSamples}` : '520'}
          </span>
          <span className="text-[9.5px] text-[#7c2d12]">Verified Real Incidents</span>
        </div>
      </div>
      ...
      {activeTab === 'architecture' && (
        <div className="flex flex-col gap-3">
          <div className="p-3 rounded-2xl border bg-white border-[#fed7aa] text-[11px] leading-relaxed">
            <div className="flex items-center gap-1.5 font-black text-[#c2410c] mb-1">
              <Cpu size={14} />
              <span>Two-Stage Segregated High-Level Architecture</span>
            </div>
            <p className="text-[#7c2d12] mb-3">
            ...
            <div className="flex flex-col gap-2 font-mono text-[10px]">
              <div className="p-2 rounded-xl border bg-[#fff7ed] border-[#fed7aa]">
                <div className="font-bold text-[#c2410c] mb-0.5">STAGE A — Spatiotemporal Persistence Engine</div>
              ...
              <div className="text-center text-[#ea580c] font-bold">↓ If Candidate Event (Sudden Surge or Low Recurrence) ↓</div>

              <div className="p-2 rounded-xl border bg-[#fff7ed] border-[#fed7aa]">
                <div className="font-bold text-[#c2410c] mb-0.5">STAGE B — Supervised ML Fire-Type Classifier</div>
                <div className="text-[#7c2d12]">
          ...
          <div className="p-3 rounded-2xl border bg-white border-[#fed7aa]">
            <h4 className="text-[11px] font-extrabold uppercase tracking-wider text-[#7c2d12] mb-2">
              Active Telemetry Stream Breakdown ({hotspots.length} Hotspots)
            </h4>
            <div className="flex flex-col gap-1.5 text-[10.5px]">
              <div className="flex justify-between items-center p-1.5 rounded-lg bg-red-50 border border-red-200">
                ...
                <span className="font-mono font-bold text-red-800">{industrialFires}</span>
              </div>
              <div className="flex justify-between items-center p-1.5 rounded-lg bg-orange-50 border border-orange-200">
                ...
                <span className="font-mono font-bold text-[#7c2d12]">{persistentSources}</span>
              </div>
              <div className="flex justify-between items-center p-1.5 rounded-lg bg-amber-50 border border-amber-200">
                <span className="font-bold text-amber-800 flex items-center gap-1.5">
                  <span className="w-2 h-2 rounded-full bg-amber-500" /> Wildfires / Vegetation Fires (Stage B)
                </span>
                <span className="font-mono font-bold text-amber-900">{wildfires}</span>
              </div>
              <div className="flex justify-between items-center p-1.5 rounded-lg bg-yellow-50 border border-yellow-200">
                <span className="font-bold text-yellow-800 flex items-center gap-1.5">
                  <span className="w-2 h-2 rounded-full bg-yellow-500" /> Agricultural Stubble Burns (Stage B)
                </span>
                <span className="font-mono font-bold text-yellow-900">{agricultural}</span>
              </div>
              <div className="flex justify-between items-center p-1.5 rounded-lg bg-purple-50 border border-purple-200">
                ...
                <span className="font-mono font-bold text-purple-900">{mining}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      {activeTab === 'evaluation' && (
        <div className="flex flex-col gap-3">
          ...
          <div className="p-3 rounded-2xl border bg-white border-[#fed7aa]">
            <h4 className="text-[11px] font-extrabold uppercase tracking-wider text-[#7c2d12] mb-1.5 flex items-center gap-1">
              <BarChart3 size={13} className="text-[#ea580c]" />
              <span>Systematic Multi-Algorithm Comparison (5-Fold Stratified CV)</span>
            </h4>
            <p className="text-[10px] text-[#7c2d12] mb-2">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-[10px] text-left border-collapse">
                <thead>
                  <tr className="border-b border-[#fed7aa] bg-[#fff7ed] text-[#7c2d12]">
                ...
                <tbody>
                  {metrics?.modelComparison?.map((row) => (
                    <tr
                      key={row.Model}
                      className={`border-b border-[#fed7aa]/60 ${
                        row.Model === 'CatBoost' ? 'bg-orange-50/80 font-bold text-[#c2410c]' : 'text-[#431407]'
                      }`}
                    >
                      <td className="p-1.5 flex items-center gap-1">
                        {row.Model === 'CatBoost' && <span className="text-[9px] px-1 py-0.2 rounded bg-orange-200 text-[#7c2d12]">Primary</span>}
                        {row.Model}
                      </td>
                      <td className="p-1.5 font-mono">{row['CV Macro F1']}%</td>
                      <td className="p-1.5 font-mono">{row['Holdout Macro F1']}%</td>
                      <td className="p-1.5 font-mono text-red-600">{row['Industrial Fire Recall']}%</td>
                    </tr>
                  )) || (
                    <tr>
                      <td colSpan={4} className="p-2 text-center text-[#7c2d12]">
          ...
          {metrics?.confusionMatrix && (
            <div className="p-3 rounded-2xl border bg-white border-[#fed7aa]">
              <h4 className="text-[11px] font-extrabold uppercase tracking-wider text-[#7c2d12] mb-1.5">
              ...
              <div className="overflow-x-auto">
                <table className="text-[9.5px] border-collapse font-mono text-center w-full">
                  <thead>
                    <tr className="border-b border-[#fed7aa] text-[#7c2d12]">
                  ...
                  <tbody>
                    {metrics.confusionMatrix.matrix.map((row, rowIdx) => (
                      <tr key={rowIdx} className="border-b border-[#fed7aa]/40">
                        <td className="p-1 text-left font-sans font-bold text-[#7c2d12]">
      ...
      {activeTab === 'leakage' && (
        <div className="p-3 rounded-2xl border bg-white border-[#fed7aa] text-[11px] leading-relaxed">
          ...
          <div className="flex flex-col gap-2.5">
            <div className="p-2.5 rounded-xl border bg-red-50/60 border-red-200">
              <div className="font-bold text-red-800 text-[10.5px] mb-1">
                1. Provenance &amp; Auditing Field Isolation
              </div>
              <p className="text-[10px] text-[#7c2d12]">
                Fields such as <code className="bg-white px-1 rounded font-mono text-red-700">incident_name</code>,{' '}
                <code className="bg-white px-1 rounded font-mono text-red-700">reference_source</code>, and FIRMS debugging metadata{' '}
                (<code className="bg-white px-1 rounded font-mono text-red-700">firms_deep_api_successes</code>,{' '}
                <code className="bg-white px-1 rounded font-mono text-red-700">firms_match</code>) are strictly isolated for dataset QA and auditing.
                They are <strong>NEVER</strong> supplied as predictor features to the model.
              </p>
            </div>

            <div className="p-2.5 rounded-xl border bg-orange-50/60 border-orange-200">
              <div className="font-bold text-[#c2410c] text-[10.5px] mb-1">
                2. Post-Event &amp; Future Temporal Leakage Exclusion
              </div>
              <p className="text-[10px] text-[#7c2d12]">
                Post-event variables like <code className="bg-white px-1 rounded font-mono text-[#c2410c]">fire_duration_days</code>,{' '}
                <code className="bg-white px-1 rounded font-mono text-[#c2410c]">spread_rate_km_day</code>, and{' '}
                <code className="bg-white px-1 rounded font-mono text-[#c2410c]">thermal_area_km2</code> only become known after an incident resolves.
      ...
      {activeTab === 'features' && (
        <div className="flex flex-col gap-2.5">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-[10.5px]">
            <div className="p-2.5 rounded-xl border bg-white border-[#fed7aa]">
              <div className="flex items-center gap-1.5 font-bold text-[#c2410c] mb-1">
                <Flame size={13} /> Thermal Telemetry (NASA FIRMS)
              </div>
              <p className="text-[#7c2d12] text-[9.5px]">
                ...
                {metrics.topFeatures.slice(0, 6).map((f) => (
                  <div key={f.feature} className="flex flex-col gap-1">
                    <div className="flex justify-between text-[10px]">
                      <span className="font-mono text-[#431407]">{f.feature.replace('num__', '').replace('cat__', '')}</span>
                      <span className="font-mono font-bold text-[#c2410c]">{f.importance.toFixed(1)}%</span>
                    </div>
                    <div className="w-full h-1.5 bg-[#fed7aa]/50 rounded-full overflow-hidden">
                      <div
